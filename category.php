<?php 

    include './models/category.php';

 ?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">
  <title>Product Management</title>
  <!-- Bootstrap CSS -->
  <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css">
  <!-- Font Awesome -->
  <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
  <!-- Custom CSS -->
  <link rel="stylesheet" href="/css/style.css">
</head>

<?php
      $cat = new category(); 
?>
<body>
  <!-- Navbar -->
  <nav class=" container navbar navbar-expand-lg navbar-light  ">
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav">
        <li class="nav-item ">
          <a class="nav-link" href="index.php" style="background-color: white; color: black; padding: 5px 10px; border-radius: 5px;">Product </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="category.php" style="background-color: blue; color: white; padding: 5px 10px; border-radius: 5px;">Categories</a>
        </li>
      </ul>
     
    </div>
  </nav>

   <!-- Search Form -->
  <form action="productsearch.php" method="get" class="form-inline d-flex justify-content-center mt-4">
    <div class="input-group">
    <input type="text" name="fsearch"  onfocus="this.value ='';" onblur="if (this.value == '') {this.value = 'Search Product';}">
      <div class="input-group-append">
        <input type="submit" name="search" value="Search" class="btn-primary" >
      </div>
    </div>
  </form>

    <div class="container">
        <div class="row">
            <div class="col-md-6">
              <p class="search-results"></p>
            </div>
            <div class="col-md-6 text-right">
              <a href="#" class="add-product"><i class="fa fa-plus"></i> </a>
            </div>
          </div>
    </div>
  

  <!-- Main Content -->
  <div class="container mt-3">
    <h3>Category List</h3>
    <table class="table table-striped table-bordered">
      <thead>
        <tr>
          <th>#</th>
          <th>Category Name</th>
          
          <th>Actions</th>
        </tr>
      </thead>
      <tbody>
        <?php
          $show_cat = $cat->show_category();
          
          if($show_cat)
          {					
            while ($result = $show_cat->fetch_assoc())
            {
           
        ?>
        <tr>
          <td><?php echo $result['catId']?></td>
          <td><?php echo $result['catName'] ?></td>
            <td>
            <a href="#" class="btn btn-sm "><i class="fa fa-edit"></i></a>
            <a href="#" class="btn btn-sm "><i class="fa fa-trash"></i></a>
            <a href="#" class="btn btn-sm "><i class="fa fa-eye"></i></a>
             <a href="#" class="btn btn-sm "><i class="fa fa-file"></i></a>
          </td>
        </tr>
        <?php
            }
          }   
        ?>
        
       
     </tbody>
    </table>
    </div>
</body>
      
</html>